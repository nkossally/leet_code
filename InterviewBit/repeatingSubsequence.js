module.exports = {
  ...
  longestSubSequence: function (A) {
    if (A.length === 0) return 0;
    let maxString = "";

    const getSubString = (selectedStr, remainder, idx) => {
        const otherIdx = remainder.indexOf(selectedStr);
        if (otherIdx !== -1 && selectedStr.length > maxString.length) {
          maxString = selectedStr;
        }

        for(let i = idx; i < remainder.length; i++){
            const newRemainder = remainder.slice(0, i) + remainder.slice(i + 1)
            getSubString(selectedStr + remainder[i], newRemainder, i)
        }
    };
    getSubString("", A, 0);
    console.log(maxString)
    return maxString.length >= 2 ? 1 : 0;
  },
};

module.exports.longestSubSequence("aaa");
